...
def conver_image_into_binary(filename):
    ...
    return photo_image
def getProfile(id):
    conn = sqlite3.connect('C:\\Users\\ththo\\Desktop\\PBL563\\AdminWeb\\db.sqlite3') 
...
def InsertRecord(people_id, checkpeople,ok):
    ...
    query = "Insert into home_checkpeople(checkpeople,ok,id_check,time,image) values('"+str(checkpeople)+"','"+str(ok)+"','"+str(people_id)+"','"+str(current_time)+ "','"+insert_photo+")"
    conn.execute(query)
    conn.commit()
    conn.close()
...
client.connect(('192.168.107.87', 1002))
